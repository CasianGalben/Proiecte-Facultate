...
struct RBTree {
	...
	int bh() {
	    if(!root)
            ...
            return countBlacks(root->left);*/

		return isNil(root) ? 0 : countBlacks(root->left);
	}
    ...
    int maxBlackKey() {
	        ...
	        while(!isNil(temp))
                ...
                if(temp->col==RBNode::BLACK && temp->key >value)
                    value= temp->key;
            temp =temp->right;
	...
	int maxRedKey() {
            ...
            int Maxim = -1;
            x= root;
            while(!isNil(x->left) || !isNil(x->right))
            {
                while(!isNil(x->right))
                {
                    if(x->col==RBNode::RED)
                        Maxim=max(x->key, Maxim);
                    x=x->right;
                }
                if( x->col == RBNode::RED)
                    Maxim= max(x->key, Maxim);
